cesEstKmenta <- function( yName, xNames, data ){
   ...
   ## Parameter vector
   result$par <- numeric( 4 )
   names( result$par ) <- c( "gamma", "alpha", "rho", "phi" )

   ## Defining gamma
   result$par[ "gamma" ] <- exp( coef( result$translog )[ "eq1_(Intercept)" ] )

   ## Defining phi
   result$par[ "phi" ] <- coef( result$translog )[ "eq1_a_1" ] +
                             coef( result$translog )[ "eq1_a_2" ]

   ## Defining alpha
   result$par[ "alpha" ] <- coef( result$translog )[ "eq1_a_1" ] /
                               result$par[ "phi" ]

   ## Defining rho
   result$par[ "rho" ] <- coef( result$translog )[ "eq1_b_1_2" ] /
      ( coef( result$translog )[ "eq1_a_1" ] * coef( result$translog )[ "eq1_a_2" ] /
         result$par[ "phi" ] )

   ## Delta method
   jacobian <- matrix( 0, nrow = length( result$par ),
      ncol = length( coef( result$translog )))
   rownames( jacobian ) <- names( result$par )
   ...
   jacobian[ "alpha", "eq1_a_1" ] <- 1 / result$par[ "phi" ] -
      coef( result$translog )[ "eq1_a_1" ] / result$par[ "phi" ]^2
   jacobian[ "alpha", "eq1_a_2" ] <-
      - coef( result$translog )[ "eq1_a_1" ] / result$par[ "phi" ]^2
   jacobian[ "rho", "eq1_a_1" ] <- coef( result$translog )[ "eq1_b_1_2" ] /
      coef( result$translog )[ "eq1_a_1" ] * coef( result$translog )[ "eq1_a_2" ] -
      coef( result$translog )[ "eq1_b_1_2" ] * result$par[ "phi" ] /
   ...
   jacobian[ "rho", "eq1_b_1_2" ] <-  result$par[ "phi" ] /
      coef( result$translog )[ "eq1_a_1" ] / coef( result$translog )[ "eq1_a_2" ]
